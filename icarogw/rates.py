...
from .conversions import detector2source_jacobian, detector2source, detector2source_jacobian_q, detector2source_jacobian_single_mass, detector2source_jacobian_MBH, detector2source_jacobian_EMRI_no_q, detector2source_jacobian_EMRI
...
class CBC_rate_m1_q(object):
    ...
    rate_wrapper: class
        Wrapper for the rate evolution model
    spin_wrapper: class
        Wrapper for the spin distribution
    scale_free: bool
        True if you want to use the model for scale-free likelihood (no R0)
    '''
    def __init__(self,cosmology_wrapper,mass_wrapper,
...
class CBC_redshift_rate_m1_q(object):
    '''
    This is a rate model that parametrizes the CBC rate per year at the detector in terms of source-frame
    masses, spin parameters and mass ratio, rate evolution times differential of comoving volume. Source-frame mass distribution,
    spin distribution and redshift distribution are summed to be independent from each other.
    ...
    q_wrapper: class
        Wrapper for the mass ratio distribution
    rate_wrapper: class
        Wrapper for the redshift distribution
    spin_wrapper: class
        Wrapper for the spin distribution
    scale_free: bool
        True if you want to use the model for scale-free likelihood (no R0)
    ...
    def log_rate_PE(self,prior,**kwargs):
        ...
        # Compute the weights for the samples Monte Carlo integral, Eq.3 of [2305.17973].
        # w = 1/prior_d dN/(dm1d dq ddL dtd) = 1/prior_d dN/(dm1s dq dz dts) 1/|J_d->s|
        # dN/(dm1s dq dz dts) = p(z) p_pop(m1s) p_pop(q|m1s)
        log_weights = self.mw.log_pdf(ms1) + self.qw.log_pdf(kwargs['mass_ratio']) + self.rw.rate.log_evaluate(z) \
        - xp.log(prior) - xp.log(detector2source_jacobian_q(z, self.cw.cosmology))
...
class EMRI_rate_no_q(object):
    ...
    def log_rate_PE(self, prior, **kwargs):
        ...
        - xp.log(prior) - xp.log(detector2source_jacobian_EMRI_no_q(z, ms1, self.cw.cosmology)) - xp.log1p(z)
...
class EMRI_rate(object):
    def __init__(self, cosmology_wrapper, mass_wrapper, secondary_wrapper, rate_wrapper, scale_free = False):

        self.cw = cosmology_wrapper
        self.m1w = mass_wrapper
        self.m2w = secondary_wrapper
        self.rw = rate_wrapper
        self.scale_free = scale_free

        if scale_free: self.population_parameters = self.cw.population_parameters+self.m1w.population_parameters+self.m2w.population_parameters+self.rw.population_parameters
        else:          self.population_parameters = self.cw.population_parameters+self.m1w.population_parameters+self.m2w.population_parameters+self.rw.population_parameters + ['R0']
    ...
    def update(self, **kwargs):
        ...
        '''
        self.cw.update(**{key: kwargs[key] for key in self.cw.population_parameters})
        self.m1w.update(**{key: kwargs[key] for key in self.m1w.population_parameters})
        self.m2w.update(**{key: kwargs[key] for key in self.m2w.population_parameters})
        self.rw.update(**{key: kwargs[key] for key in self.rw.population_parameters})
    ...
    def log_rate_PE(self, prior, **kwargs):
        ...
        # Convert m1 from detector to source frame.
        ms1 = kwargs['mass_1'] / (1.+z)
        ms2 = kwargs['mass_2'] / (1.+z)
        log_dVc_dz = xp.log( self.cw.cosmology.dVc_by_dzdOmega_at_z(z) * 4*xp.pi )

        # Compute the weights for the samples Monte Carlo integral, Eq.3 of [2305.17973].
        # w = 1/prior_d dN/(dm1d dm2d ddL dtd) = 1/prior_d dN/(dlog(m1s) dm2s dVc dts) 1/|J_d->s| 1/1+z dVc/dz
        # dN/(dlog(m1s) dm2s dVc dts) = R(z) p_pop(log(m1s)) p_pop(m2s)
        log_weights = self.m1w.log_pdf(xp.log10(ms1)) + self.m2w.log_pdf(ms2) + self.rw.rate.log_evaluate(z) + log_dVc_dz \
